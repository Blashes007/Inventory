...
function Purchaselist() {
  ...
  async function PurchaseDetails() {
    try {
      let result = await fetch(
        "https://api.iotech.com.np/api/purchaselist",
        {
          method: "POST",
          headers: {
  ...
  useEffect(()=>{
    ...
    async function DeletePurchase() {
        try {
          let result = await fetch("https://api.iotech.com.np/api/deletepurchasedetails", {
            method: "POST",
            headers: {
              "Content-Type": "application/json0",
              Accept: "application/json",
             ...
             <TableContainer component={Paper}>
                <Table sx={{ minWidth: 1150 }} aria-label="simple table">
                  <TableHead>
                      ...
                      <TableRow>
                        <StyledTableCell align="right">Purchase ID</StyledTableCell>
                        ...
                        <StyledTableCell align="right">Vat No</StyledTableCell>
          ...
          <TableBody>
            {tableData.map((row) => (
              <StyledTableRow key={row.purchase_id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                <TableCell align="right">{row.purchase_id}</TableCell>
